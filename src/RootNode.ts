...
import { values, reduce, isString, keyBy, isPlainObject, isEmpty, concat } from 'lodash';
import {
  GraphQLFieldConfig,
  parse,
  FieldDefinitionNode,
...
export default class RootNode {
  ...
  public buildSchema() {
    ...
    const objectTypeDefs = values<GraphQLObjectTypeConfig<any, any>>(this.objectTypeMap)
      .map(typeConfig => {console.log(typeConfig); return new GraphQLObjectType(typeConfig);});
  ...
  private buildType<TypeDefType = TypeDefinitionNode>(typeDef: string | TypeDefType): {name: string, def: TypeDefType} {
    ...
    return {name, def: this.defBuilder.buildType(typeDefNode)};
  }
}
